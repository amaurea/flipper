...
class power2D:
    ...
    def powerVsThetaInAnnulus(self,lLower,lUpper,deltaTheta=2.0,powerOfL=0,\
        ...
        if show:
            pylab.plot(binnedTheta,numpy.log(numpy.abs(binnedPA)))
            if fitSpline:
...
#pylab.show()
    ...
    def createKspaceMask(self, verticalStripe=None,horizontalStripe=None,slantStripeLxLy=None,\
        ...
        if verticalStripe!=None:
            ...
            #print idx
            mask[:,idx] = 0.
        if horizontalStripe!=None:
            idy = numpy.where((self.ly<horizontalStripe[1]) & (self.ly > horizontalStripe[0]))
            mask[idy,:] = 0.
    ...
    def plot(self,log=False,colorbar=True,title='',powerOfL=0,pngFile=None,show=True,zoomUptoL=None,\
             ...
             drawVerticalLinesAtL = None, valueRange=None, colorbarLabel=None,TryColor=None):
        """
        ...
        if drawCirclesAtL !=None:
            for ell in drawCirclesAtL:
                ...
                if len(drawCirclesAtL)<5:
                    pylab.text(ell*numpy.cos(numpy.pi/5.),ell*numpy.sin(numpy.pi/5.),\
                               'l=%d'%numpy.int(ell),rotation=-45,horizontalalignment = 'center',\
                               verticalalignment='bottom',fontsize=12)
                    
        if drawVerticalLinesAtL!=None:
            for ell in drawVerticalLinesAtL:
                pylab.axvline(ell,color='k')
        ...
        if TryColor !=None:
            ...
            cmap = colors.ListedColormap(['white','b','g','r','c','m'])
            im = pylab.imshow(p,origin="down",cmap=cmap,extent=[numpy.min(self.lx),numpy.max(self.lx),numpy.min(self.ly),numpy.max(self.ly)],aspect='equal',vmin=vmin,vmax=vmax, interpolation='nearest')
    
        else:
            im = pylab.imshow(p,origin="down",extent=[numpy.min(self.lx),numpy.max(self.lx),numpy.min(self.ly),numpy.max(self.ly)],aspect='equal',vmin=vmin,vmax=vmax, interpolation='nearest')
...
def powerFromFFT(ft,ft2=None,trimAtL=None):
    """
